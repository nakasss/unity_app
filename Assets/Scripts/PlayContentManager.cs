...
public class PlayContentManager : MonoBehaviour {
	private static readonly string baseURL = "https://storage.googleapis.com/scopic-mobile-app-storage/videos/";
	private string videoName = "sample_movie";
	private float videoDuration = 54.0f; //TODO : It have to ge got by API and it has to be ms.
	...
	public void InitPlayPage (string loadFilePath, float  videoDurationArg) {
		progressTimer = 0.0f;
		//Progress Bar
		...
		if (Application.platform == RuntimePlatform.Android || Application.platform == RuntimePlatform.IPhonePlayer) {
			videoFilePath = baseURL + videoName + ".mp4";
		} else {
			videoFilePath = baseURL + videoName + ".ogv";
		}
		if (videoDurationArg != 0)
			videoDuration = videoDurationArg;
		if (loadFilePath == null) loadFilePath = videoFilePath;
		LoadVideo(loadFilePath);
	...
	void LoadVideo (string filePath) {
		...
		#if UNITY_EDITOR
		StartCoroutine(LoadDeskMovie(filePath));
		#elif UNITY_IPHONE || UNITY_ANDROID
		LoadMobileMovie(filePath);
		#endif
	}
